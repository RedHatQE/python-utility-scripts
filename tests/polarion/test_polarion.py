...
EXIT_CODE_ERROR_MESSAGE = "{exit_code} does not match expected value."
...
def test_polarion_requirement_no_args():
    ...
    assert result.exit_code == 1, EXIT_CODE_ERROR_MESSAGE.format(exit_code=result.exit_code)
...
def test_polarion_requirement():
    ...
    LOGGER.debug(f"Result output: {result.output}, exit code: {result.exit_code}, exceptions: {result.exception}")
    assert result.exit_code == 0, EXIT_CODE_ERROR_MESSAGE.format(exit_code=result.exit_code)
